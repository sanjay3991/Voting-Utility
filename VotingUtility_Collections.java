...
import newlaptopClint.ElectionDatabase;


public class VotingUtility_Collections extends ElectionDatabase
{
	...
	public void choice() {
		...
		try {
			choice=sc.nextInt();
			ArrayList<String> data=getreaddata(choice);
			if(choice==1)
			{
				...
				for(int i=0;i<data.size();i++) {
					if(VoterIdNo.equals(data.get(i))) 
						{
						System.out.println("Congrats your database is present");
						flag=true;
						EvmMachine();
